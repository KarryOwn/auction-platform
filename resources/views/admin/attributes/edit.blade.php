<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Attribute') }}: {{ $attribute->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900" x-data="{ type: '{{ old('type', $attribute->type) }}' }">
                    <form method="POST" action="{{ route('admin.attributes.update', $attribute) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="type" name="type" x-model="type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm bg-gray-100" required disabled>
                            ...
                            <div>
                                ...
                                <x-text-input id="unit" class="block mt-1 w-full" type="text" name="unit" :value="old('unit', $attribute->unit)" placeholder="e.g., kg, cm, GB" />
                                <x-input-error :messages="$errors->get('unit')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="sort_order" :value="__('Sort Order')" />
                                <x-text-input id="sort_order" class="block mt-1 w-full" type="number" name="sort_order" :value="old('sort_order', $attribute->sort_order)" />
                                <x-input-error :messages="$errors->get('sort_order')" class="mt-2" />
                            </div>

                            <div class="md:col-span-2" x-show="type === 'select'">
                                <x-input-label for="options" :value="__('Options (JSON array)')" />
                                <textarea id="options" name="options" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm font-mono text-sm" rows="3" placeholder='["Red", "Blue", "Green"]'>{{ old('options', json_encode($attribute->options)) }}</textarea>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select id="category_ids" name="category_ids[]" multiple class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm h-48">
                                    @php
                                        $selectedCategories = old('category_ids', $attribute->categories->pluck('id')->toArray());
                                    @endphp
                            ...
                            <div class="md:col-span-2 flex gap-6">
                                <label for="is_filterable" class="inline-flex items-center">
                                    <input id="is_filterable" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_filterable" value="1" {{ old('is_filterable', $attribute->is_filterable) ? 'checked' : '' }}>
                                ...
                                <label for="is_required" class="inline-flex items-center">
                                    <input id="is_required" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_required" value="1" {{ old('is_required', $attribute->is_required) ? 'checked' : '' }}>
