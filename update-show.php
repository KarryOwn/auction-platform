...
@if(request('layout') === 'minimal')
    ...
    <body class="bg-gray-50 font-sans antialiased h-screen flex flex-col pt-0">
        <div class="bg-white border-b px-4 py-3 sticky top-0 flex items-center shadow-sm z-10">
            <a href="{{ route('messages.index') }}?layout=minimal" class="mr-3 text-gray-500 hover:text-gray-900 border rounded p-1 shadow-sm shrink-0">
        ...
        <div class="bg-white p-3 border-t">
            <form action="{{ route('messages.store', \$conversation) }}" method="POST" class="flex items-center space-x-2">
                @csrf
